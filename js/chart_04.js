...
var globaldata, split, allYears, allNames, collectNames;
...
function datasort (data) {
    ...
    collectNames = d3.nest()
        .key(function (d) {return d.geschlecht;})
        .key(function (d) {return d.vorname})
        .rollup(function (d) {
          return (d3.sum(d, function (v) {return v.anzahl }))
        })
        .map(data);  
    ...
    // for (name in collectNames["weiblich"]) {
    //   allNames.weiblich.push({vorname: name, anzahl: collectNames["weiblich"].name});
    // };

    for (name in collectNames["weiblich"]) {
      allNames.weiblich.push({vorname: name, anzahl: collectNames["weiblich"][name]});
    };
    for (name in collectNames["männlich"]) {
      allNames.männlich.push({vorname: name, anzahl: collectNames["männlich"][name]});
    };

    //console.log("Alle Data:", data);
    console.log("Alle Namen:", allNames);
    console.log("Weibliche Namen:", allNames["weiblich"]);


    var years = [];

    for(var jahrgang in allYears){
       years[allYears.length] = jahrgang;
    }
    console.log("Jahrgänge Gesamt:", allYears);
...
if (currentYear === "All") {
    ...
    names = allNames[currentGender]
      .filter(function(d) { return d.anzahl > 20; })
      .sort(function(a,b) { return b.anzahl - a.anzahl; });;

    } else {
    names = split[currentGender][currentYear]
